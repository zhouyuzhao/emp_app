define(function(require){
	...
	Model.prototype.massageClick = function(event){
		...
		AJ.RS({service:"TodoList",method:"getUsingRequestNoteInfo",scope:this,params:params,
			success:function(result){
				var jsonData = JSON.stringify(result.data);
			   	url = require.toUrl('./UsingNoteView.w');
				windowDialog.open({src:url,data:jsonData});
				//justep.Shell.showPage(require.toUrl('./UsingNoteView.w'), {"data":JSON.stringify(result.data)});
			} 		
		});
	};
